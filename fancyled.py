...
class fancyled:
     ...
     def sparkle(self): #runs sparkle function
        print("sparkle")
        rand = random.randrange(1, 6, 1) #randomly selects a pin
        #uptime = random.randrange(0, 5, 1)#randomly selects a time
        #uptime = uptime/500 #divides time to get a smaller number
        uptime = .01
       # print(uptime)
        if rand == 1:
            self.fancy1.value= True
